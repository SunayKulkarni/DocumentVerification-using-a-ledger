...
## Working Project Quick Start

This repository now includes a fully working Python CLI implementation in [main.py](main.py) for local blockchain-based document issuance and verification.
...
python main.py issue test.pdf --owner "Alice" --note "initial issue"
python main.py verify test.pdf
python main.py validate
python main.py show --last 5
```

### Command Summary

- `init`: create blockchain file (`chain.json` by default)
- `issue`: register a document hash and metadata in a new block
- `verify`: verify a file against hashes stored on-chain
- `validate`: validate blockchain integrity (hash links and block hashes)
- `show`: display blockchain blocks

### Optional Arguments

- `--chain <path>`: use a custom blockchain JSON file
- `--document-id <id>`: set/filter document id during issue/verify
